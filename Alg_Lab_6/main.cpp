#include <SFML/Graphics.hpp>
#include <iostream>
#include <vector>
#include <queue>
#include <cmath>
#include <algorithm>
#include <random>

using namespace std;
using namespace sf;

const int GRID_SIZE = 10; // –азмер пол€ NxN
const int CELL_SIZE = 80;  // –азмер одной €чейки в пиксел€х
const int WINDOW_SIZE = GRID_SIZE * CELL_SIZE; // размер окна

// “ипы €чеек
enum CellType {
    EMPTY, // пустота
    OBSTACLE, // ограждение
    START,// начало
    END,// конец
    PATH,// путь
    VISITED// посещЄнные
};

// структура €чейки
struct Cell {
    int x, y; // координаты
    CellType type; // тип €чейки
    int f, g, h; // ƒл€ алгоритма A*
    Cell* parent; // указатель на родителей

    Cell(int x, int y) : x(x), y(y), type(EMPTY), f(0), g(0), h(0), parent(nullptr) {}

    // дл€ сравнени€ €чеек
    bool operator==(const Cell& other) const {
        return x == other.x && y == other.y;
    }
};



//функци€ дл€ вычислени€ эвристики (манхэттенское рассто€ние)
int heuristic(const Cell& a, const Cell& b) {
    return abs(a.x - b.x) + abs(a.y - b.y);
}

// функци€ дл€ проверки, находитс€ ли €чейка в пределах сетки
bool isValid(int x, int y) {
    return (x >= 0 && x < GRID_SIZE && y >= 0 && y < GRID_SIZE);
}


//функци€ дл€ создани€ сетки
std::vector<std::vector<Cell>> make_grid(int GRID_SIZE) {
    vector<vector<Cell>> grid;
    for (int x = 0; x < GRID_SIZE; x++) {
        std::vector<Cell> row;
        for (int y = 0; y < GRID_SIZE; y++) {
            Cell cell(x, y);
            row.push_back(cell);
        }
        grid.push_back(row);
    }
    return grid;
}

//функци€ дл€ отрисовки сетки
void draw_grid(sf::RenderWindow& win, std::vector<std::vector<Cell>>& grid) {
    for (int i = 0; i < GRID_SIZE; ++i) {
        for (int j = 0; j < GRID_SIZE; ++j) {
            RectangleShape cell(Vector2f(CELL_SIZE - 1, CELL_SIZE - 1));
            cell.setPosition(i * CELL_SIZE, j * CELL_SIZE);

            switch (grid[i][j].type) {
            case EMPTY: cell.setFillColor(Color::White); break;
            case OBSTACLE: cell.setFillColor(Color::Black); break;
            case START: cell.setFillColor(Color::Green); break;
            case END: cell.setFillColor(Color::Red); break;
            case PATH: cell.setFillColor(Color::Blue); break;
            case VISITED: cell.setFillColor(Color::Cyan); break;
            }

            win.draw(cell);
        }
    }
}

// сортирует €чейки так, чтобы первым всегда была €чейка с наименьшим значением f
struct CompareCells {
    bool operator()(Cell const* c1, Cell const* c2) {
        return c1->f > c2->f;
    }
};

// функци€ восстановлени€ пути
void reconstruct_path(Cell* current) {
    if (current && current->type == END) {
        current = current->parent;
    }
    while (current != nullptr && current->type != START) {
        current->type = PATH;
        current = current->parent;
    }
}

//алгоритм A*
void a_star_algorithm(std::vector<std::vector<Cell>>& grid, Cell* start, Cell* end) {
    for (int x = 0; x < GRID_SIZE; x++) {
        for (int y = 0; y < GRID_SIZE; y++) {
            grid[x][y].parent = nullptr;
            grid[x][y].g = 999999;
            if (grid[x][y].type == PATH || grid[x][y].type == VISITED) {
                grid[x][y].type = EMPTY;
            }
        }
    }

    // очередь дл€ хранени€ доступных €чеек
    std::priority_queue<Cell*, std::vector<Cell*>, CompareCells> open_list;

    // двумерный массив дл€ посещенных и обработанных €чеек
    std::vector<std::vector<Cell*>> closed_list(GRID_SIZE, std::vector<Cell*>(GRID_SIZE, nullptr));

    // начальна€ клетка
    start->g = 0;
    start->f = heuristic(*start, *end);
    open_list.push(start);

    // основной цикл алгоритма
    while (!open_list.empty()) {
        // берЄтс€ €чейка с наименьшим f
        Cell* n = open_list.top();
        open_list.pop();

        // если конец, то строитс€ путь
        if (*n == *end) {
            reconstruct_path(&grid[end->x][end->y]);
            return;
        }

        // если €чейка находитс€ в закрытом списке Ч пропускаем еЄ
        if (closed_list[n->x][n->y] != nullptr) continue;

        // добавл€ем €чейку в закрытый список
        closed_list[n->x][n->y] = n;

        // помечаем пустую €чейку как посещенную
        if (n->type == EMPTY) {
            n->type = VISITED;
        }

        // смещени€ дл€ 4-х соседних €чеек 
        int dx[] = { 0, 0, 1, -1 };
        int dy[] = { 1, -1, 0, 0 };

        for (int i = 0; i < 4; i++) {
            int nx = n->x + dx[i];
            int ny = n->y + dy[i];

            // проверка 
            if (!isValid(nx, ny) || grid[nx][ny].type == OBSTACLE || closed_list[nx][ny] != nullptr) {
                continue;
            }

            // рассчитываем предполагаемую стоимость этого соседа
            Cell* m = &grid[nx][ny];
            int tentative_g = n->g + 1;

            // если найден более короткий путь
            if (tentative_g < m->g) {
                m->parent = n;
                m->g = tentative_g;
                m->h = heuristic(*m, *end);
                m->f = m->g + m->h; // обновл€ем стоимость

                // добавл€ем соседа в открытй список
                open_list.push(m);
            }
        }
    }
}

// генераци€ пол€ по заданию
void generate_grid(vector<vector<Cell>>& grid) {
    grid[3][1].type = START; //начало
    grid[7][9].type = END; // конец
    // координаты непроходимых клеток
    vector<Vector2i> obstacles = {
        {0,3},
        {1,3},

        {3,0}, {3,2}, {3,3},
        {4,1}, {4,2},
        {5,3}, {5,8},
        {6,8},
        {7,1}, {7,4}, {7,5}, {7,8},
        {8,0}, {8,4}, {8,7}, {8,9},
        {9,1}, {9,3}, {9,7}, {9,8}
    };

    // обозначение непроходимых клеток
    for (const auto& p : obstacles) {
        if (p.x < GRID_SIZE && p.y < GRID_SIZE) {
            grid[p.x][p.y].type = OBSTACLE;
        }
    }

}

int main() {
    // окно и поле по заданию
    RenderWindow window(VideoMode(WINDOW_SIZE, WINDOW_SIZE), "A*");
    vector<vector<Cell>> grid = make_grid(GRID_SIZE);
    generate_grid(grid);

    // клетки начала и конца
    Cell* start = nullptr;
    Cell* end = nullptr;

    for (auto& row : grid) {
        for (auto& cell : row) {
            if (cell.type == START) {
                start = &grid[cell.x][cell.y];
            }
            if (cell.type == END) {
                end = &grid[cell.x][cell.y];
            }
        }
    }

    while (window.isOpen()) {
        // обработка событий
        Event event;
        while (window.pollEvent(event)) {
            // закрытие окна
            if (event.type == Event::Closed) {
                window.close();
            }
            if (event.type == Event::KeyPressed) {
                if (event.key.code == Keyboard::Space) {
                    a_star_algorithm(grid, start, end);
                }
            }
        }

        window.clear(); // очистка экрана
        draw_grid(window, grid); // отрисовка всех €чеек
        window.display(); // отображение
    }
    return 0;
}